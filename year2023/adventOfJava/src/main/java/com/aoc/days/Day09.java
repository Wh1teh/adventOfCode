...
public class Day09 extends Day {
    ...
    public String solveFirstPart() {
        return "" + getPredictions(true);
    }

    /**
     * Foreach history, puts interpolations in the stack, then pulls from the stack
     * and extrapolates right or left side to get a prediction
     * 
     * @param first isFirstPart?
     * @return Sum of predictions
     */
    private int getPredictions(boolean first) {
        List<Integer> predictions = new ArrayList<>();
        for (String line : LINES) {
            Stack<List<Integer>> histories = new Stack<>();
            List<Integer> history = Parsers.intsList(line);

            List<Integer> interHistory = new ArrayList<>();
            do {
                histories.add(new ArrayList<>(history));
                interHistory.clear();

                for (int i = 1; i < history.size(); i++) {
                    interHistory.add(history.get(i) - history.get(i - 1));
                }

                history.clear();
                history.addAll(interHistory);
            } while (!allZeroes(interHistory));

            int latest = 0;
            while (!histories.isEmpty()) {
                List<Integer> makkara = histories.pop();
                latest = first ? latest + makkara.get(makkara.size() - 1) : makkara.get(0) - latest;
            }
