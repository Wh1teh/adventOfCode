...
import com.aoc.util.GenericWrapper;
...
public class Day11 extends Day {
    ...
    // TODO: there is probably a not-O(n^2) way of doing this
    private long getSumDumb(final int multiplier) {
        GenericWrapper<Map<String, Set<Integer>>, List<Coordinates>> universeData = getExpandedRowsAndCols(LINES);
        Set<Integer> rowsWithGalaxies = universeData.getFirst().get("rowsWithGalaxies");
        Set<Integer> colsWithGalaxies = universeData.getFirst().get("colsWithGalaxies");
        List<Coordinates> coordinates = universeData.getSecond();

        long sum = 0;
        for (Coordinates fromCoordinates : coordinates) {
            for (Coordinates toCoordinates : coordinates) {
                final int fromY = fromCoordinates.getYs();
                final int fromX = fromCoordinates.getXs();

                final int toY = toCoordinates.getYs();
                final int toX = toCoordinates.getXs();

                final int expandedRows = countCrossings(fromY, toY, rowsWithGalaxies);
                final int expandedCols = countCrossings(fromX, toX, colsWithGalaxies);

                sum += (Math.abs(fromY - toY) + (expandedRows * multiplier - expandedRows))
                        + (Math.abs(fromX - toX) + (expandedCols * multiplier - expandedCols));
            }
        }
        return sum / 2;
    }

    private int countCrossings(int start, int end, Set<Integer> dimensionWithGalaxies) {
        if (start > end) {
            int temp = start;
            start = end;
            end = temp;
        ...
        int count = 0;
        for (; start <= end; start++) {
            if (!dimensionWithGalaxies.contains(start)) {
                count++;
            }
        }
        return count;
    ...
    private GenericWrapper<Map<String, Set<Integer>>, List<Coordinates>> getExpandedRowsAndCols(List<String> lines) {
        Map<String, Set<Integer>> expansions = new HashMap<>();
        List<Coordinates> coordinates = new ArrayList<>();

        Set<Integer> rowsWithGalaxies = new HashSet<>();
        Set<Integer> colsWithGalaxies = new HashSet<>();
        expansions.put("rowsWithGalaxies", rowsWithGalaxies);
        expansions.put("colsWithGalaxies", colsWithGalaxies);
        ...
        return new GenericWrapper<>(expansions, coordinates);
    }
