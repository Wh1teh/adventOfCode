...
public class Day10 extends Day {
    ...
    public String solveFirstPart() {
        Map<Coordinates, Character> nodes = new HashMap<>();
        Coordinates start = getNodes(nodes);
        Graph<Coordinates> graph = buildGraph(nodes);

        return "" + graph.bfs(start).size() / 2;
    }

    private final Set<Character> CAN_UP = new HashSet<>(Arrays.asList('S', '|', 'L', 'J'));
    private final Set<Character> CAN_DOWN = new HashSet<>(Arrays.asList('S', '|', 'F', '7'));
    private final Set<Character> CAN_RIGHT = new HashSet<>(Arrays.asList('S', '-', 'L', 'F'));
    private final Set<Character> CAN_LEFT = new HashSet<>(Arrays.asList('S', '-', '7', 'J'));

    private Coordinates getNodes(Map<Coordinates, Character> nodes) {
    ...
    private Graph<Coordinates> buildGraph(Map<Coordinates, Character> nodes) {
        Graph<Coordinates> graph = new Graph<>();

        for (Map.Entry<Coordinates, Character> node : nodes.entrySet()) {
            Coordinates coordinates = node.getKey();
            final int Y = coordinates.getYs();
            final int X = coordinates.getXs();
            Character pipe = node.getValue();

            Character up = nodes.get(new Coordinates(Y - 1, X));
            Character down = nodes.get(new Coordinates(Y + 1, X));
            Character right = nodes.get(new Coordinates(Y, X + 1));
            Character left = nodes.get(new Coordinates(Y, X - 1));
            if (up != null && CAN_UP.contains(pipe) && CAN_DOWN.contains(up)) {
                graph.addEdge(coordinates, new Coordinates(Y - 1, X));
            }
            if (down != null && CAN_DOWN.contains(pipe) && CAN_UP.contains(down)) {
                graph.addEdge(coordinates, new Coordinates(Y + 1, X));
            }
            if (right != null && CAN_RIGHT.contains(pipe) && CAN_LEFT.contains(right)) {
                graph.addEdge(coordinates, new Coordinates(Y, X + 1));
            }
            if (left != null && CAN_LEFT.contains(pipe) && CAN_RIGHT.contains(left)) {
                graph.addEdge(coordinates, new Coordinates(Y, X - 1));
            }
        }
    ...
    public String solveSecondPart() {
        ...
        for (int row = 0; row < grid.length; row++) {
            ...
            for (int col = 0; col < grid[row].length; col++) {
                boolean tile = grid[row][col];
                Character pipe = line.charAt(col);
        ...
        // Path path = Paths.get("src/main/java/com/aoc/data/day10_visual.txt");
        // try {
        // Files.writeString(path, visualization.toString());
        // } catch (IOException e) {
    ...
    private boolean[][] convertToBooleanGrid(Set<Coordinates> visits, List<String> lines) {
        boolean[][] grid = new boolean[lines.size()][lines.get(0).length()];
        for (Coordinates coordinates : visits) {
            grid[coordinates.getYs()][coordinates.getXs()] = true;
        }
        return grid;
    }
