...
public class Day04 extends Day {
    ...
    private int playGame(String line) {
        ...
        String[] winningRaw = temp[0].split(" ");
        String[] playedRaw = temp[1].split(" ");

        List<Integer> winning = new ArrayList<>();
        for (String num : winningRaw) {
            if (!num.isBlank())
                winning.add(Integer.parseInt(num));
        }

        List<Integer> played = new ArrayList<>();
        for (String num : playedRaw) {
            if (!num.isBlank())
                played.add(Integer.parseInt(num));
        }

        int sum = 0;
        for (Integer playedNum : played) {
            for (Integer winningNum : winning) {
                if (playedNum == winningNum) {
                    sum = sum == 0 ? 1 : sum * 2;
                }
            }
        }
    ...
    private void countTotalWinnings(Map<Integer, Long> winnings, int index) {
        ...
        Long cardWinnings = winnings.get(index);

        for (int i = 1; i <= cardWinnings && index + i < winnings.size(); i++) {
            sum += winnings.get(index + i) + 0;
        }

        winnings.put(index, sum);
    }

    private Long playGameRedux(String line) {
        String[] temp = line.split(":");
        temp = temp[1].split("\\|");
